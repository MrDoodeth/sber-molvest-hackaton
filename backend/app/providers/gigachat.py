...
Калибруй шкалу так:
- 0.0 — нет понятного запроса или пользователь явно просит подключить оператора;
...
Сначала определи, просит ли пользователь передать обращение оператору, специалисту
или живому человеку, включая свободные формулировки этой просьбы. Если просит,
верни operator_requested=true и confidence=0. Это правило имеет высший приоритет
над всеми другими правилами. Во всех остальных случаях верни
operator_requested=false. Порог эскалации из CURRENT SETTINGS не является значением
