...
DEFAULT_PROMPTS: dict[PromptType, str] = {
    PromptType.USER_SUPPORT: (
        ...
        "факта, задай короткий уточняющий вопрос и одновременно предложи то, что "
        "пользователь может безопасно проверить сам. Не проси оператора только из-за "
        "отсутствия статьи в базе знаний: backend эскалирует сам по явной просьбе "
        "пользователя или после трёх последовательных низких confidence. "
    ...
    PromptType.KNOWLEDGE_CARD: (
        ...
        "версии, причины и пункты меню. Если факт не зафиксирован, укажи «Не указано "
        "в тикете». Не добавляй Markdown, комментарии, вложенные объекты, "
