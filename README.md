...
GigaChat оценивает уверенность ответа -> GigaChat формирует потоковый ответ через
SSE -> пользователь продолжает диалог
...
Сообщение пользователя -> анализ приложенного снимка экрана через GigaChat (если
приложен) -> поиск по базе знаний -> построение единого `GenerationContext` ->
первый вызов GigaChat с `ConfidenceAssessment` -> проверка уверенности -> второй
вызов GigaChat со всем контекстом -> передача ответа пользователю потоком через SSE.
