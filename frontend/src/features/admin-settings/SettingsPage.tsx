...
function validate(
  ...
  const errors: string[] = [];
  if (settings.gigachatContextRatio < 0 || settings.gigachatContextRatio > 1)
    errors.push("Размер контекста GigaChat должен быть от 0% до 100%.");
  if (settings.embeddingContextRatio < 0 || settings.embeddingContextRatio > 1)
    errors.push("Контекст Embeddings должен быть от 0% до 100%.");
  if (
    settings.operatorEscalationThreshold < 0 ||
    settings.operatorEscalationThreshold > 1
  )
    errors.push("Порог оператора должен быть от 0% до 100%.");
  const totalBudget = Math.floor(
    gigachatContextLimit * settings.gigachatContextRatio,
  );
  if (
    !Number.isInteger(settings.gigachatMaxOutputTokens) ||
    settings.gigachatMaxOutputTokens <= 0 ||
    settings.gigachatMaxOutputTokens >= totalBudget
  )
    errors.push(
      "Максимальный ответ должен быть целым положительным числом меньше общего budget GigaChat.",
    );
  if (!Number.isInteger(settings.ragTopK) || settings.ragTopK < 1)
    errors.push("rag_top_k должен быть целым числом не меньше 1.");
  return errors;
}
...
export default function SettingsPage() {
  ...
  const settings = useQuery({
    ...
    queryFn: ({ signal }) => settingsApi.settings(signal),
  });
  useEffect(() => {
  ...
  const selectedModel = settings.data?.availableModels?.find(
    (model) => model.id === form?.activeModel,
  );
  const gigachatLimit =
    selectedModel?.contextLimit ??
    settings.data?.capabilities.gigachatContextLimit ??
    0;
  const embeddingLimit = settings.data?.capabilities.embeddingContextLimit ?? 0;
  const totalBudget = form
    ? Math.floor(gigachatLimit * form.gigachatContextRatio)
    : 0;
  const inputBudget = form ? totalBudget - form.gigachatMaxOutputTokens : 0;
  const embeddingBudget = form
    ? Math.floor(embeddingLimit * form.embeddingContextRatio)
    : 0;
  ...
  const dirty = Boolean(
    ...
    (form.activeModel !== settings.data.activeModel ||
      ...
      form.operatorEscalationThreshold !==
        settings.data.operatorEscalationThreshold),
  );
  ...
  if (settings.isPending)
    return <PageLoader label="Загружаем настройки и limits" />;
  if (settings.isError)
    return (
      <ErrorState
  ...
  if (!form || !settings.data)
    return (
      <EmptyState
        ...
        description="Backend не вернул typed settings response."
      />
    );
      ...
      <div className="mx-auto max-w-6xl">
            ...
            <div className="grid gap-5 p-5">
              ...
              >
                <Select
                  ...
                  onChange={(event) =>
                    setForm({ ...form, activeModel: event.target.value })
                  }
                >
                  {settings.data.availableModels?.map((model) => (
              ...
              <RangeField
                label="Размер скользящего окна GigaChat"
                value={form.gigachatContextRatio}
                onChange={(value) =>
                  setForm({ ...form, gigachatContextRatio: value })
                }
                preview={`${formatPercent(form.gigachatContextRatio)} от ${gigachatLimit.toLocaleString("ru-RU")} = ${totalBudget.toLocaleString("ru-RU")} tokens`}
              />
              <Field
                label="Максимальный размер ответа"
                hint={`Под вход остаётся ${Math.max(inputBudget, 0).toLocaleString("ru-RU")} tokens.`}
              >
                <Input
                  type="number"
                  min="1"
                  step="1"
                  value={form.gigachatMaxOutputTokens}
                  onChange={(event) =>
                    setForm({
                      ...form,
                      gigachatMaxOutputTokens: Number(event.target.value),
                    })
                  }
                />
              </Field>
              <RangeField
                label="Порог подключения оператора"
                value={form.operatorEscalationThreshold}
                onChange={(value) =>
                  setForm({ ...form, operatorEscalationThreshold: value })
                }
                preview={`${formatPercent(form.operatorEscalationThreshold)}: ниже этого confidence тикет эскалируется`}
              />
            </div>
          </Card>
          <Card className="overflow-hidden">
            ...
            <div className="grid gap-5 p-5">
              <RangeField
                ...
                preview={`${formatPercent(form.embeddingContextRatio)} от ${embeddingLimit.toLocaleString("ru-RU")} = ${embeddingBudget.toLocaleString("ru-RU")} tokens`}
              />
              <Field
                label="rag_top_k"
              ...
              >
                <Input
                  ...
                  value={form.ragTopK}
                  onChange={(event) =>
                    setForm({ ...form, ragTopK: Number(event.target.value) })
                  }
                />
              </Field>
              <div className="rounded-2xl border border-molvest-200 bg-molvest-50 p-4">
                ...
                <dl className="mt-4 grid gap-3 text-xs">
                  <div className="flex justify-between gap-4">
                    <dt className="text-stone-500">Общий GigaChat</dt>
                    <dd className="font-black text-molvest-900">
                      {totalBudget.toLocaleString("ru-RU")}
                    </dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-stone-500">Вход GigaChat</dt>
                    <dd className="font-black text-molvest-900">
                      {Math.max(inputBudget, 0).toLocaleString("ru-RU")}
                    </dd>
                  </div>
                  <div className="flex justify-between gap-4">
                    <dt className="text-stone-500">Вход BGE-M3</dt>
                    <dd className="font-black text-molvest-900">
                      {embeddingBudget.toLocaleString("ru-RU")}
                    </dd>
                  </div>
                </dl>
              </div>
        ...
        {errors.length > 0 && (
          ...
          >
            <div className="flex items-center gap-2 text-sm font-bold text-red-900">
              <Gauge className="size-4" /> Проверьте значения
            </div>
            <ul className="mt-2 grid gap-1 text-xs leading-5 text-red-800">
              {errors.map((error) => (
        ...
        <div className="mt-5 flex justify-end">
          <Button
            ...
            onClick={() => save.mutate()}
          >
            <Save className="size-4" /> Сохранить все настройки
          </Button>
        </div>
      </div>
    </div>
