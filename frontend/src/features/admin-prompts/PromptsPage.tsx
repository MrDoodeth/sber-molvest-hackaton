    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-molvest-600">Runtime instructions</p><h1 className="mt-1 text-3xl font-black tracking-[-0.025em] text-molvest-950">System Prompts</h1><p className="mt-2 text-sm text-stone-500">Два независимых промпта применяются к следующим вызовам GigaChat без перезапуска.</p></div>
          {dirty && <Badge tone="warning">Есть несохранённые изменения</Badge>}
        </header>
        <Card className="mt-6 overflow-hidden">
          <div className="border-b border-stone-100 p-4 sm:p-5"><Tabs value={type} onChange={requestTab} ariaLabel="Тип System Prompt" items={[{ value: "user_support", label: "User Support" }, { value: "operator_gigachat", label: "Operator AI GigaChat" }]} /></div>
          ...
          {prompt && (
            <div className="grid lg:grid-cols-[minmax(0,1fr)_17rem]">
              ...
              <aside className="border-t border-stone-100 bg-stone-50 p-5 lg:border-l lg:border-t-0">
                <div className="flex size-10 items-center justify-center rounded-2xl bg-indigo-100 text-indigo-700">{type === "user_support" ? <Bot className="size-5" /> : <ShieldCheck className="size-5" />}</div>
