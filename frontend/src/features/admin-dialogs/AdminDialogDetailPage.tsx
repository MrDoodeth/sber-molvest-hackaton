    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
           ...
           <Card className="p-4"><span className="flex items-center gap-2 text-xs font-bold text-stone-400"><Sparkles className="size-4" /> System Prompt</span><strong className="mt-2 block text-sm text-stone-900">{audit.systemPrompt ? "Снимок сохранён" : "—"}</strong></Card>
         </div>
         <Card className="mt-3 p-5"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-stone-400"><Sparkles className="size-4" /> Полный prompt финального решения</div><pre className="mt-3 max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-xl bg-stone-50 p-4 text-xs leading-5 text-stone-700">{audit.systemPrompt || "Снимок prompt для этого диалога отсутствует."}</pre></Card>
