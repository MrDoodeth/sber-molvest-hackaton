...
export default function LoginPage() {
  ...
  const me = queryClient.getQueryData<{ role: Role; displayName: string }>(
    queryKeys.me(),
  );
        ...
        <section className="my-auto grid items-center gap-10 py-12 lg:grid-cols-[0.88fr_1.12fr] lg:gap-16">
          ...
          <div className="rounded-[28px] border border-[#dbe3f0] bg-white/90 p-4 shadow-[0_30px_70px_rgb(31_58_100/18%)] backdrop-blur sm:p-6">
            <div className="mb-5 flex items-end justify-between gap-4 px-1">
              ...
              {me && (
                <Button size="sm" onClick={() => navigate(rolePath[me.role])}>
              ...
              {roleCards.map(
                ({ role, title, description, icon: Icon, accent }) => (
                  ...
                  >
                    ...
                    {login.isPending && selectedRole === role ? (
                      <span className="text-xs font-bold text-molvest-700">
                        Входим…
                      </span>
                    ) : (
