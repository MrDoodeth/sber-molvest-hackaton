              ...
              {documents.data && documents.data.items.length > 0 && (
               <div className="w-full">
                 <table className="w-full table-fixed text-left text-sm">
                    <thead className="bg-stone-50 text-[10px] font-extrabold uppercase tracking-[0.12em] text-stone-500"><tr><th className="w-[42%] px-2 py-3 sm:px-5">Документ</th><th className="w-[18%] px-2 py-3 sm:px-4">Включён</th><th className="w-[22%] px-2 py-3 sm:px-4">Обновлён</th><th className="w-[18%] px-2 py-3 sm:px-5">Индекс</th></tr></thead>
                     <tbody className="divide-y divide-stone-100">{documents.data.items.map((document) => <tr key={document.id} className="hover:bg-molvest-50/40"><td className="min-w-0 px-2 py-4 sm:px-5"><div className="flex min-w-0 items-center gap-2 sm:gap-3"><a href={document.downloadUrl || knowledgeApi.documentDownloadUrl(document.id)} download className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-stone-100 text-stone-500 transition hover:bg-molvest-100 hover:text-molvest-700 focus-visible:outline-2 focus-visible:outline-molvest-500" title={`Скачать ${document.title}`} aria-label={`Скачать ${document.title}`}><FileText className="size-4" /></a><p className="min-w-0 truncate font-bold text-stone-900" title={document.title}>{truncateTitle(document.title, 20)}</p></div></td><td className="px-2 py-4 sm:px-4"><Switch checked={document.isEnabled} label={`${document.isEnabled ? "Выключить" : "Включить"} документ ${document.title}`} disabled={toggleDocument.isPending} onChange={(next) => toggleDocument.mutate({ id: document.id, isEnabled: next })} /></td><td className="px-2 py-4 text-xs text-stone-500 sm:px-4">{formatRelativeDate(document.updatedAt)}</td><td className="px-2 py-4 sm:px-5"><div className="flex max-w-full flex-wrap items-center gap-1"><Badge className="max-w-full break-all text-[10px]" tone={indexTone[document.indexStatus]}>{document.indexStatus}</Badge>{document.indexStatus === "failed" && <IconButton type="button" className="size-8 text-red-700 hover:bg-red-50" aria-label={`Повторить индексацию ${document.title}`} disabled={reindex.isPending} onClick={() => reindex.mutate(document.id)}><RefreshCw className="size-3.5" /></IconButton>}</div></td></tr>)}</tbody>
                </table>
              </div>
            )}
          </Card>
