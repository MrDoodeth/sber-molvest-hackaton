...
import { BookOpen, FilePlus2, FileText, FolderPlus, MoreHorizontal, Pencil, RefreshCw, Trash2, Upload } from "lucide-react";
...
import { Badge, Button, Card, ConfirmDialog, EmptyState, ErrorState, Field, IconButton, Input, Modal, PageLoader, Switch, useToast } from "../../shared/ui";
...
export default function KnowledgePage() {
  ...
  const filters: DocumentFilters = { sectionId: selectedSectionId || undefined };
  ...
  const isCaseJournal = selectedSection?.isSystem && selectedSection.name === "Журнал обращений";
  ...
  const upload = useMutation({
    mutationFn: (input: { file: File; sectionId: string }) => knowledgeApi.uploadDocument(input),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: queryKeys.kb.documents(filters) });
      void queryClient.invalidateQueries({ queryKey: queryKeys.kb.sections() });
          ...
          <Card className="min-h-72 overflow-hidden">
            ...
            <div className="grid gap-1 p-2">
              {sections.data?.map((section) => (
                   <div key={section.id} className={cn("group rounded-xl border p-3 transition", selectedSectionId === section.id ? "border-molvest-400 bg-molvest-50" : "border-transparent hover:bg-[#f1f4fb]")}>
                  <div className="flex items-start justify-between gap-2">
                    <button type="button" onClick={() => setSearchParams({ section: section.id })} className="min-w-0 flex-1 text-left focus-visible:outline-2 focus-visible:outline-molvest-500">
                       <p className="truncate text-sm font-bold text-black">{section.name}</p>
                       <p className="mt-1 text-[11px] text-slate-400">{section.documentCount ?? 0} документов</p>
                    </button>
                   ...
                   <div className="mt-2 flex items-center gap-1 opacity-100 lg:opacity-0 lg:transition lg:group-hover:opacity-100">
                     {!section.isSystem && <><IconButton type="button" className="size-8 text-stone-400 hover:bg-white hover:text-molvest-700" onClick={() => { setName(section.name); setRenameSection(section); }} aria-label={`Переименовать ${section.name}`}><Pencil className="size-3.5" /></IconButton><IconButton type="button" className="size-8 text-stone-400 hover:bg-red-50 hover:text-red-700" onClick={() => setSectionAction({ section, kind: "delete" })} aria-label={`Удалить ${section.name}`}><Trash2 className="size-3.5" /></IconButton></>}
          ...
          <Card className="min-w-0 overflow-hidden">
             <div className="flex flex-wrap items-end justify-between gap-3 border-b border-[#dbe3f0] p-4 sm:p-5">
              <div><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-molvest-600">Documents</p><h2 className="mt-1 text-xl font-bold text-molvest-950">{selectedSection?.name || "Выберите раздел"}</h2>{selectedSection && !selectedSection.isEnabled && <p className="mt-1 text-xs font-semibold text-amber-800">Master switch выключен: документы временно не участвуют в RAG.</p>}</div>
               <div className="flex items-center gap-2"><input ref={fileInputRef} className="sr-only" type="file" accept={KB_ACCEPT} onChange={(event) => handleFileChange(event.target.files?.[0])} /><Button pending={upload.isPending} disabled={!canUpload} onClick={chooseFile}><Upload className="size-4" /> Загрузить</Button></div>
            </div>
            ...
            {documents.isError && <ErrorState description={documents.error.message} onRetry={() => void documents.refetch()} />}
              {documents.isSuccess && documents.data.items.length === 0 && <EmptyState icon={<FilePlus2 className="size-8" />} title={isCaseJournal ? "Одобренных кейсов пока нет" : "Документов нет"} description={isCaseJournal ? "Одобренные карточки решений появятся здесь после модерации." : "Нажмите «Загрузить», чтобы выбрать файл и запустить индексацию."} />}
              {documents.data && documents.data.items.length > 0 && (
               <div className="w-full">
                  <table className="w-full table-fixed text-left text-sm">
                     <thead className="bg-[#f7f9fd] text-[10px] font-extrabold uppercase tracking-[0.12em] text-slate-500"><tr><th className="w-[42%] px-2 py-3 sm:px-5">Документ</th><th className="w-[18%] px-2 py-3 sm:px-4">Включён</th><th className="w-[22%] px-2 py-3 sm:px-4">Обновлён</th><th className="w-[18%] px-2 py-3 sm:px-5">Индекс</th></tr></thead>
                      <tbody className="divide-y divide-[#edf1f9]">{documents.data.items.map((document) => <tr key={document.id} className="hover:bg-[#fafcff]"><td className="min-w-0 px-2 py-4 sm:px-5"><div className="flex min-w-0 items-center gap-2 sm:gap-3"><a href={document.downloadUrl || knowledgeApi.documentDownloadUrl(document.id)} download className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-[#eef2fa] text-slate-500 transition hover:bg-molvest-100 hover:text-molvest-700 focus-visible:outline-2 focus-visible:outline-molvest-400" title={`Скачать ${document.title}`} aria-label={`Скачать ${document.title}`}><FileText className="size-4" /></a><p className="min-w-0 truncate font-bold text-black" title={document.title}>{truncateTitle(document.title, 20)}</p></div></td><td className="px-2 py-4 sm:px-4"><Switch checked={document.isEnabled} label={`${document.isEnabled ? "Выключить" : "Включить"} документ ${document.title}`} disabled={toggleDocument.isPending} onChange={(next) => toggleDocument.mutate({ id: document.id, isEnabled: next })} /></td><td className="px-2 py-4 text-xs text-slate-500 sm:px-4">{formatRelativeDate(document.updatedAt)}</td><td className="px-2 py-4 sm:px-5"><div className="flex max-w-full flex-wrap items-center gap-1"><Badge className="max-w-full break-all text-[10px]" tone={indexTone[document.indexStatus]}>{document.indexStatus}</Badge>{document.indexStatus === "failed" && <IconButton type="button" className="size-8 text-red-700 hover:bg-red-50" aria-label={`Повторить индексацию ${document.title}`} disabled={reindex.isPending} onClick={() => reindex.mutate(document.id)}><RefreshCw className="size-3.5" /></IconButton>}</div></td></tr>)}</tbody>
                </table>
              </div>
            )}
          </Card>
