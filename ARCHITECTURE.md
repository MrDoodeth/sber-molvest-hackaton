...
Для нашего MVP UI сохраняем простое правило:

```text
до 1 runtime attachment на пользовательское сообщение
```

Этого достаточно для основного сценария screenshot 1С и не заставляет нас усложнять обработку нескольких текстовых документов.
...
Так как для MVP мы ограничиваем runtime attachment одним файлом на сообщение, отдельную сложную orchestration-логику вокруг `get_file_content` сейчас не добавляем.
