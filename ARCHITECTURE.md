    ...
    - **Критичное ограничение Freemium — 1 поток генерации.** Все GigaChat generation, vision и Files API операции проходят через единый re-entrant `GenerationGate` внутри процесса. Обычный пользовательский turn использует до **двух последовательных GigaChat generation-call** (`candidate answer → judge`), а turn со screenshot — до **трёх** (`screenshot parse → candidate answer → judge`). Candidate не публикуется до завершения judge: при достаточной уверенности он отдаётся buffered SSE-потоком, при низкой уверенности выбрасывается, заменяется одним вопросом для уточнения или приводит к эскалации. Ручной шаблон оператора выполняется отдельным generation-call.
    ...
    Если confidence ниже порога, готовый candidate не публикуется: backend либо задаёт
    полезный уточняющий вопрос (не более двух раундов), либо переводит обращение оператору.
    ...
    **Компоненты:**
    ...
    - **Dialog Service** — состояние диалога/история, роутинг в RAG/Vision/Escalation.
        - **RAG Engine** — локальная векторизация (`EmbeddingProvider`) → hybrid retrieval из Qdrant → evidence для candidate/judge pipeline.
        - **Vision / Attachments Handler** — хранит runtime attachment, для screenshot выполняет отдельный GigaChat parse (`extracted_text + visual_summary`) до RAG, затем переиспользует тот же `file_id` в candidate/judge pipeline. Text attachments также передаются в generation-вызовы с `function_call="auto"`.
        - **Dialog decision flow** — `DialogService` применяет `AnswerAssessment` и `operator_escalation_threshold`, задаёт ограниченное уточнение или переводит обращение в операторскую очередь.
    ...
    **Латентность (бюджет на <5 сек из ТЗ):**
    ...
    | GigaChat до первого токена | измеряем на API; user UI получает buffered chunks после candidate/judge |
    ...
    **Масштабирование:** на MVP отдельная очередь задач не нужна. Индексация запускается через FastAPI `BackgroundTasks` / простой worker, а документы в `uploaded/processing` восстанавливаются при старте. Один пользовательский AI-turn глобально защищён PostgreSQL advisory admission lock, а локальный `TurnCoordinator` добавляет process-local guard. `GenerationGate` сериализует provider work только внутри одного процесса; распределённая очередь для всех generation/file/vision вызовов при нескольких replicas остаётся Roadmap.
    ...
    ### Candidate/judge: latency и буферизация

    Новый runtime специально использует два последовательных GigaChat-call для обычного
    AI-turn:

    ```text
    candidate answer stream → backend buffer → structured judge → decision
    ```

    Пользовательский SSE начинается только после judge. Это увеличивает time-to-first-token
    и расход токенов, но не позволяет показать сомнительный candidate. Митигация:

    - judge возвращает маленький structured `AnswerAssessment`;
    - используется небольшой технический `max_tokens` для judge;
    - тот же `X-Session-ID` позволяет GigaChat кэшировать совпадающий prompt-prefix;
    - при низком confidence candidate отбрасывается, а повторный generation-call для
      пользовательского ответа не нужен;
    - фактическую среднюю latency обязательно измеряем на demo-наборе относительно
      целевого KPI `<5 сек`.
    ...
    CALL #1 — candidate answer, stream buffered on backend
    + system prompt
    + current message
    ...
    + attachments
            ↓
    CALL #2 — structured AnswerAssessment judge
    {confidence, clarification_useful, clarification_question, escalation_required}
            ↓
    Decision Policy
    ├── confidence >= threshold → persist and emit buffered candidate answer
    ├── low confidence + useful question → persist clarification (max 2 rounds)
    └── otherwise → switch to operator_support
    ```

    Candidate answer никогда не отправляется пользователю до завершения judge.
    При явной просьбе оператора LLM pipeline обходится: backend сразу выставляет
    confidence `0` и переводит Dialog в `operator_support`.
    ...
    #### Этап 2B. Judge не подтвердил готовый ответ

    После candidate answer backend вызывает structured judge с контрактом:
    ...
    Candidate пользователю ещё не отправлен. Decision policy работает так:

    ```python
    if confidence >= operator_escalation_threshold:
        persist_and_emit_buffered_candidate()
    elif (
        ...
        persist_clarification_question()
    else:
        switch_to_operator_mode()
    ```

    При низком confidence candidate отбрасывается. Уточнение допускается максимум два
    раза; если уточнение не помогает, judge не предлагает его или явно требует
    специалиста, backend в одной транзакции переводит Dialog в `operator_support` и
    сохраняет system Message с confidence и внутренним snapshot `Message.sources`.
    ...
    `GenerationContextService` используется для user candidate/judge, ручного operator template
    ...
    Во время каждого AI user turn значение обновляется результатом judge. Это confidence
    уже сформированного candidate answer, а не предварительная оценка способности модели
    ответить.
    ...
    ┌──────────────────┼─────────────────────┐
    │                  │                     │
    confidence >=      low + useful          otherwise
    threshold           clarification         operator_support
    │                  │
    ↓                  ↓
    buffered answer     clarification question
    → persist + SSE     → persist + SSE
    ```

    Candidate answer не публикуется до завершения judge. При достаточной уверенности
    backend сохраняет его и отправляет пользователю накопленные chunks через SSE. При
    низкой уверенности candidate отбрасывается.
    ...
    Judge получает тот же полный контекст и готовый candidate answer. Если confidence ниже
    порога, он может предложить ровно один конкретный вопрос. Уточнение разрешено максимум
    два раза; после уже заданного уточнения confidence должен вырасти. Если вопрос не
    полезен, отсутствует, повторяется, лимит исчерпан или judge требует специалиста,
    backend переводит Dialog в `operator_support`.

    #### Поведение после подключения оператора

    В `operator_support` новые пользовательские сообщения не запускают candidate/judge или
    ...
    Успешный turn требует двух последовательных generation-вызовов, поэтому latency выше,
    но сомнительный candidate не показывается пользователю. Screenshot добавляет отдельный
    structured parse до retrieval. Structured output также используется для явной
    генерации Knowledge Card; обычный candidate создаётся без этого вызова.
    ...
    sequenceDiagram
        ...
        API->>G: Call #1 candidate answer (buffered)
        G-->>API: candidate text
        API->>G: Call #2 structured AnswerAssessment judge
        G-->>API: {confidence, clarification, escalation}
        API-->>U: SSE confidence event

        alt confidence >= threshold
            API-->>U: buffered candidate as SSE assistant_token...
    ...
    В user `ai_support` pipeline модель сначала выполняет candidate generation, затем
    отдельный judge оценивает корректность уже сформированного candidate. Только после
    этого backend публикует ответ, задаёт уточнение или эскалирует. В operator pipeline
    отдельный generation-вызов формирует шаблон ответа; он не отправляется пользователю
    автоматически.

    Анализ screenshot выполняется отдельным structured vision-вызовом до retrieval.

    `confidence` оценивается отдельным structured-вызовом judge в `ai_support` после
    формирования candidate и до его публикации пользователю. Structured-вызовы также
    ...
    ```text
    CALL #1 → candidate answer, buffered on backend
    CALL #2 → structured AnswerAssessment judge
    ```
    ...
    Единственное осознанное разделение user generation-логики — candidate generation,
    затем короткий structured judge. При достаточной уверенности buffered candidate
    публикуется; при низкой уверенности он заменяется clarification или эскалацией.
    Ручной operator template выполняется отдельным вызовом без user judge.
    ...
    Для user candidate в текущем MVP используем цепочку:

    ```text
    GigaChat candidate SSE
        ↓
    FastAPI backend buffer
        ↓
    AnswerAssessment judge
        ↓
    FastAPI SSE proxy
        ↓
    Frontend
    ...
    Пользователь видит chunks только после завершения candidate и judge; после этого
    накопленный ответ быстро отдаётся через SSE. Так frontend не получает сомнительный
    candidate и не должен парсить потоковый JSON.

    Важно: generation stream занимает доступный GigaChat-поток до завершения model call,
    а `GenerationGate` освобождается до отправки уже сохранённого buffered ответа через
    SSE.

    Документация:
    - https://developers.sber.ru/docs/ru/gigachat/guides/response-token-streaming

    ### Structured Output: screenshot, answer judge и Knowledge Card

    Structured Output используется в трёх явных местах:

    1. screenshot parse возвращает `extracted_text` и `visual_summary`;
    2. answer judge оценивает готовый candidate через
       `AnswerAssessment(confidence, clarification_useful, clarification_question,
       escalation_required)`;
    ...
    `json_schema → function_calling`. Candidate answer и operator template остаются
    обычным plain text. Streaming Structured Output и incremental JSON parser не нужны.

    Важно: answer judge вызывается **после** candidate generation. Он оценивает именно
    текст candidate, а не способность модели ответить до генерации.
    ...
    rag_top_k evidence
            ↓
    CALL #1 — candidate answer (buffered)
            ↓
    CALL #2 — structured AnswerAssessment judge
            ↓
    threshold / clarification policy
            ↓
    buffered answer, clarification или operator_support
    ```
    ...
    В MVP пользовательское сообщение может содержать до 10 вложений, но не более одного изображения. Поэтому единственный screenshot проходит отдельный parse, а документы передаются в candidate и judge generation-вызовы. Ограничение «одно изображение на message» дополнительно соблюдается адаптером GigaChat.
    ...
    Candidate/judge flow использует уже полный контекст:
    ...
    Сам screenshot можно повторно передать в candidate/judge generation-вызовы через уже
    загруженный `file_id`.
    ...
    7. Candidate generation возвращает обычный текст без JSON и служебных статусов.
    8. Judge возвращает structured `AnswerAssessment` и оценивает именно candidate.
    ...
    **Ключевое архитектурное правило проекта:**
    ...
    Structured Output используется для **Call #2 — judge готового candidate**:
    ...
    assessment = await judge.ainvoke(judge_messages)
    ```

    До judge **Call #1** выполняется обычным `llm.astream(...)`, но его chunks только
    буферизуются backend. Если confidence достаточный, накопленный candidate сохраняется
    и отправляется через SSE; иначе candidate не публикуется, а decision policy выбирает
    clarification или `operator_support`.
    ...
    Candidate-call выполняется через `astream()`, но chunks сначала собираются в backend
    buffer. Judge выполняется обычным `ainvoke()` и возвращает маленький Pydantic-объект
    до публикации candidate пользователю.
    ...
    ```text
    user candidate → plain text stream → backend buffer
    Call #2 → structured AnswerAssessment → backend decision
    approved candidate → plain text SSE → frontend
    ...
    **Не включаем одновременно:**
    ...
    - [S1], [S2]
            ↓
    OUTPUT:
    candidate → judge → approved buffered answer или clarification/operator
    ```


    Screenshot preprocessing нужен для того, чтобы до candidate/judge generation найти
    ...
    RAG evidence и source snapshots используются backend внутри candidate/judge и
    ...
    (buffered SSE) (assistant_done)  wait operator
    ```
