...
Для plain-text файлов больше 512 KB backend не передаёт оригинал в Files API: он
берёт ограниченный UTF-8 preview из собственного storage, добавляет начало и конец
файла в текущий вопрос и подбирает его размер под input budget модели. Это сохраняет
возможность разобрать большой лог или текст без переполнения контекста.

Для остальных файлов либо для ответа GigaChat `422` текущий runtime flow:
